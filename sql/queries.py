...
# Connect to your existing marketing.db file
conn = sqlite3.connect("marketing.db")
cur = conn.cursor()

# Run your CREATE VIEW script
cur.executescript("""
-- Overall response rate
CREATE VIEW IF NOT EXISTS v_response_rate AS
SELECT AVG(Response * 1.0) AS response_rate
FROM customers;

-- Campaign acceptance rates
CREATE VIEW IF NOT EXISTS v_campaign_rates AS
SELECT
...
CREATE VIEW IF NOT EXISTS v_segment_response AS
...
CREATE VIEW IF NOT EXISTS v_spend_by_demo AS
...
CREATE VIEW IF NOT EXISTS v_channels_high_value AS
SELECT
...
CREATE VIEW IF NOT EXISTS v_underserved AS
