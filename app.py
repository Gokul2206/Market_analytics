...
from data_prep import prep  # your cleaning pipeline

st.set_page_config(page_title="Marketing Analytics Dashboard", layout="wide")

st.title("📊 Marketing Analytics Dashboard")

# --- Load cleaned data with spinner ---
with st.spinner("Loading and cleaning data..."):
    df = prep("data/marketing_campaign_data.csv")

# --- Sidebar filters ---
st.sidebar.header("Filters")
selected_countries = st.sidebar.multiselect("Country", df["Country"].unique(), default=df["Country"].unique())
selected_age_bands = st.sidebar.multiselect("Age Band", df["Age_Band"].unique(), default=df["Age_Band"].unique())
selected_income_bands = st.sidebar.multiselect("Income Band", df["Income_Band"].unique(), default=df["Income_Band"].unique())
